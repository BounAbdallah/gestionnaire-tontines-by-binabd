...
import type { Tontine, MonthlyReport, MonthData, User } from "../types"
...
const ReportsView: React.FC<ReportsViewProps> = ({ tontines, currentUser }) => {
  ...
  const generateReport = async (): Promise<void> => {
    ...
    try {
      const report = await db.generateMonthlyReport(selectedTontineId, Number.parseInt(selectedMonth), currentUser.id)
      if (report) {
        const monthLabel =
        ...
        alert("Impossible de générer le rapport pour cette tontine")
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
        ...
        {tontines.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-slate-500">Vous n'avez pas encore de tontines pour générer des rapports</p>
          </div>
        ) : (
          <>
